...
session_start();
 ...
 mysqli_select_db($db,'bidtable2');
 


 $alert="";
 
 header('refresh:20;b2.php');
 
 
 $sq="SELECT * FROM bidtable2";
 $record=mysqli_query($db,$sq);

 $sql1="SELECT * FROM bidtable2 ORDER BY no DESC LIMIT 1";
 ...
 $lastprice=mysqli_fetch_assoc($lastone);
  ...
  $s="SELECT * FROM bidtable2 WHERE price='$subprice'";
  ...
  $num=mysqli_num_rows($result);
	...
	$l=" INSERT INTO bidtable2(username,price) VALUE ('$username','$subprice')";
...
<span id="hours">&nbsp &nbsp &nbsp{{hours}}</span><span id="min">{{minutes}}</span><span id="sec">{{sec}}</span> <span id="ampm">{{ampm}}</span>
  ...
  <div class="row align-items-center my-5">
      ...
      <div class="col-lg-7">
        <img class="img-fluid rounded mb-4 mb-lg-0" src="images/dagger.jpeg" alt="">
      </div>
      <!-- /.col-lg-8 -->
      <div class="col-lg-5">
        <h1 class="font-weight-light">Ancient Temple Dagger </h1>
        <p> </p>
      </div>
      <!-- /.col-md-4 -->
...
<br>
 ...
 echo '<br><br>'. ' This bidding last until mentioned time. Winner will be announced only after completion of the whole Bidding . ';
